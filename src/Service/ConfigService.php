...
class ConfigService
{
	...
	public const DEFAULT_DOES_NOT_EXIST_MESS	= 'grin_way_service.exception.file_does_not_exist';
	...
	*/
	...
	public function __construct(
		...
		protected readonly string $grinWayServiceProjectDir,
		/* INTO GrinWayServiceExtension::setParametersFromBundleConfiguration()
			[
				[
		...
		*/
		...
		protected array $grinWayServicePackageFilenames,
		#[Autowire(service: 'Symfony\Contracts\Translation\TranslatorInterface')]
		protected $grinWayServiceT,
	) {
		...
		$this->grinWayServicePackageFilenames = \array_map(
			static fn($el) => [...$defaultPackageFilenames, ...$el],
			$grinWayServicePackageFilenames,
		);
	...
	private function getConfigFromPackageFilenames(
		...
		if (empty($this->grinWayServicePackageFilenames)) return null;
		
		$id = $this->getUniqPackId($filename, $packRelPath);
		
		foreach($this->grinWayServicePackageFilenames as $packageConfig) {
			[
	...
	private function initPackageFilenameDataByPackageFilenames(): void {
		...
		if (empty($this->grinWayServicePackageFilenames)) return;
	...
	private function checkFileExisting(
		...
		if (!\is_file($absPathToFile)) {
			throw new \Exception($this->grinWayServiceT->trans(
				$mess,
				[
					'%path%' => $absPathToFile,
				],
