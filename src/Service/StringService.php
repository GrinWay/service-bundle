...
class StringService
{
    ...
    */
    public function isPathLikeNetworkIpV4(
        ...
        return \preg_match(
            '~^(?:' . $this->grinWayServiceSlashOfIpRegex . ')?' . $this->grinWayServiceIpV4Regex . '.*$~',
            \trim($path),
        ) === 1;
    }
    ...
    */
    public function isNetworkIpV4HasExactlyDoubleShashInTheBeginning(
        ...
        return \preg_match(
            '~^[/\\\]{2}' . $this->grinWayServiceIpV4Regex . '.*$~',
            \trim($path),
        ) === 1;
    }
    ...
    */
    public function getYearBySubstr(
        ...
        $grinWayServiceYearRegex = $this->grinWayServiceYearRegex;

        if ($fullYear) {
            $grinWayServiceYearRegex = $this->grinWayServiceYearRegexFull;
        }

        \preg_match(
            '~(?<year>' . $grinWayServiceYearRegex . ')~',
            $yearSubstr,
            $matches,
        );
    ...
    */
    public function getMoreSimilarOnCurrentYearBySubstr(
        ...
        \preg_match_all(
            '~(?<year>' . $this->grinWayServiceYearRegex . ')~',
            $yearSubstr,
            $matches,
        );
    ...
    */
    public function getRoot(
        ...
        if ($isNetworkPath) {
            ...
            $matches = [];
            \preg_match(
                '~^(?<' . $ipRootName . '>' . $this->grinWayServiceSlashOfIpRegex . '' . $this->grinWayServiceIpV4Regex . ').*~',
                \trim($path),
                $matches,
            );
    ...
    */
    ...
    private function isOnlyNetworkPath(
        ...
        return \preg_match('~^' . $this->grinWayServiceSlashOfIpRegex . $this->grinWayServiceIpV4Regex . '$~', \trim($path)) === 1;
    }

    private function isNetworkPath(
        string $path,
    ): bool {
        return \preg_match('~^' . $this->grinWayServiceSlashOfIpRegex . '.*$~', \trim($path)) === 1;
    }
